...
public class StaticEvolutionaryPR implements Algorithm<CPDPInstance, CPDPSolution> {
    ...
    public StaticEvolutionaryPR(Constructive<CPDPInstance, CPDPSolution> c, Improvement<CPDPSolution> ls, int populationSize, int refsetSize) {
        this.c = c;
        this.ls = ls;
    ...
    public Result execute(CPDPInstance instance) {
        ...
        for (int i = 0; i < refsetSize/2; i++) {
            refSet.add(populationList.remove(population.size()-1));
        }
        ...
        for (int i = 0; i < refsetSize; i++) {
            for (int j = i+1; j < refsetSize; j++) {
                CPDPSolution iprSol = ipr.combine(populationList.get(i), populationList.get(j));
                if (iprSol != null) {
                    ls.improve(iprSol);
                    checkBest(iprSol);
                }

                CPDPSolution eprSol = epr.combine(populationList.get(i), populationList.get(j));
                if (eprSol != null) {
                    ls.improve(eprSol);
                    checkBest(eprSol);
                }
    ...
    public String toString() {
        return this.getClass().getSimpleName()+"(" + c +", " + ls + ", " + refsetSize + ")";
    }
}
